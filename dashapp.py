...
@app.callback(
    Output("match_drop", "options"),
    Output("match_drop", "value"),
...
def get_player_matches(player_id):
    ...
    return (
        gameDF.loc[gameDF.match_id.isin(player_matches), ["match_id", "match_name"]]
...
@app.callback(
    ...
    [Input("match_drop", "value"), Input("player_search_drop", "value")],
    prevent_initial_call=True,
)
def output_plot(match_id, player_id):
    ...
    if (ctx.triggered_id == "match_drop") & (match_id is not None):
        selDF = passDF.loc[
            (passDF.player_id == player_id) & (passDF.match_id == match_id)
        ].dropna(subset=["end_location_x"])
        arrow_list = [
            dict(
                x=x_end,
                y=y_end,
        ...
        fig.add_trace(
            go.Scatter(
                ...
                text=selDF.xT_str,
                customdata=selDF.id,
                hovertemplate="xT = %{text}<extra></extra>",
                mode="markers",
                marker_size=10,
            )
        )
